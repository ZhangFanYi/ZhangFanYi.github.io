# 项目 06:Qwen3-30B 训练优化(Megatron-SFT 框架 Prof 分析,性能翻倍)

## 概述

针对客户 Qwen3-30B-A3B(Megatron-SFT 框架,TP4×EP4×PP2)训练性能不足的问题,从零搭建 Megatron 框架下的 prof 抓取流程,分析 trace 定位到**大量小 GEMM 算子**,通过开启 grouped GEMM 融合使**训练性能翻倍**。

## 背景

- 客户环境:DTK 2504.2 + Python 3.10 + torch 2.5
- 性能表现:我方 5 次迭代约 7 分钟,友商(华为 910B)约 3.5 分钟,差距一倍
