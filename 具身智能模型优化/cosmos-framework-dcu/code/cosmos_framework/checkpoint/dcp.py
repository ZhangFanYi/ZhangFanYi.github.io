...
class DistributedCheckpointer(AbstractCheckpointer):
    ...
    def __init__(
        ...
        # PyTorch DCP performs save-plan exchange with object collectives.
        # On HCU/RCCL, the NCCL-backed gather_object path can fail even though
        # the actual tensor writes are valid.  Keep the training process group
        # for FSDP and lazily create a CPU/Gloo metadata group for DCP saves.
        # The background async saver already initializes with Gloo and therefore
        # does not need a second group.
        self._dcp_metadata_process_group: dist.ProcessGroup | None = None
    ...
    def save_state_dict_worker(self, to_save_dict: Dict[str, Tuple[Any, str]], checkpoint_file: str) -> None:
        dcp_metadata_process_group = self._get_dcp_metadata_process_group()
