...
class CheckpointConfig(pydantic.BaseModel):
    ...
    """Additional config overrides for this registered checkpoint."""
...
class CheckpointOverrides(ConfigOverrides):
    ...
    * Local path: /path/to/checkpoint
    ...
    def _build_checkpoint(self, checkpoints: dict[str, CheckpointConfig]):
        ...
        checkpoint_experiment_overrides: tuple[str, ...] = ()
        if self.checkpoint_path in checkpoints:
            ...
            checkpoint_experiment_overrides = checkpoint.experiment_overrides
        ...
        experiment_overrides = [
            *checkpoint_experiment_overrides,
...
class SetupArgs(ABC, CheckpointArgs, ParallelismArgs, GuardrailArgs):
    ...
    quantization_include_regex: list[str] = pydantic.Field(default_factory=list)
...
class SetupOverrides(ABC, CheckpointOverrides, ParallelismOverrides, GuardrailOverrides):
    ...
    """Number of measured generations per input batch."""
    ...
    quantization_method: str | None = None
    """Optional model quantization method (``mxfp8``, ``nvfp4``, or ``fp8``)."""
    quantization_include_regex: list[str] = pydantic.Field(default_factory=list)
    """Regexes selecting parameters for quantization."""
    quantization_exclude_regex: list[str] = pydantic.Field(default_factory=list)
    """Regexes excluding parameters from quantization."""
