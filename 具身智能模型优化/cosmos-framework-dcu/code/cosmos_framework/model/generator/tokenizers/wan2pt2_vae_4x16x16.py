...
# Opt-in only.  The default path must keep the historical NCDHW layout because
# the VAE is also used by existing AOT packages whose signatures/guards assume
# contiguous tensors.  The MIOpen NDHWC suggestion is a separate backend knob
# and is intentionally not used as the feature switch here.
_USE_CHANNELS_LAST_3D = os.getenv("COSMOS_WAN_VAE_CHANNELS_LAST_3D", "0").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def _to_channels_last_3d(t: torch.Tensor) -> torch.Tensor:
    if _USE_CHANNELS_LAST_3D and t.dim() == 5:
...
def _contiguous_clone(t: torch.Tensor) -> torch.Tensor:
    ...
    if _USE_CHANNELS_LAST_3D and t.dim() == 5:
        if t.is_contiguous(memory_format=torch.channels_last_3d):
            return t.clone(memory_format=torch.preserve_format)
        ...
        if self.mode == "upsample3d":
            if feat_cache is not None:
                ...
                if feat_cache[idx] is None:
                    ...
                    feat_cache[idx] = _to_channels_last_3d(
                        torch.zeros(b, c, CACHE_T, h, w, device=x.device, dtype=x.dtype)
                    )  # [B,C,2,H,W]
                    feat_idx[0] += 1
                else:
...
class AvgDown3D(nn.Module):
    ...
    def forward(
        ...
        if _USE_CHANNELS_LAST_3D:
            # The original view/permute path is measurably slower for
            # channels_last_3d tensors on HCU.  pixel_unshuffle performs the
            # spatial rearrangement through the backend's native kernel; the
            # following reshape only restores the historical channel order
            # [C, temporal_offset, height_offset, width_offset].  Keep this
            # branch opt-in with the rest of the NDHWC path because it is not
            # faster for the default NCDHW layout.
            if self.factor_s > 1:
                ...
                x = x.view(
                    B,
                    T,
                    C * self.factor_s * self.factor_s,
                    H // self.factor_s,
                    W // self.factor_s,
                )
                x = x.permute(0, 2, 1, 3, 4)
            _, C_with_space, T, H, W = x.shape
            if self.factor_t > 1:
                x = x.reshape(
                    B,
...
class WanVAE_(nn.Module):
    ...
    def _encode_chunk_impl(
        ...
        return _to_channels_last_3d(self._normalize_latent(mu, scale)), feat_cache
    ...
    def encode(self, x: torch.Tensor, scale: tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        **AOT compilation:**
        ...
        def _run_chunk(
            ...
            # Ensure the layout expected by the selected backend.  The default
            # remains contiguous NCDHW; the opt-in path keeps chunks NDHWC so
            # Conv3d does not pay a layout conversion at every layer.
            memory_format = torch.channels_last_3d if _USE_CHANNELS_LAST_3D else torch.contiguous_format
...
class WanVAE:
    def __init__(
        ...
        if _USE_CHANNELS_LAST_3D:
            ...
            log.info("WanVAE: enabled channels_last_3d for Conv3d weights and activations")
    ...
    def encode(self, videos: torch.Tensor) -> torch.Tensor:
        ...
        with self.context:
            ...
            videos = _to_channels_last_3d(videos)
