...
def _optimizer_cls(
    ...
    **optimizer_kwargs: Any,
    ...
    - ``"apexfusedadamw"``: Apex's ``apex.optimizers.FusedAdam`` with
      ``adam_w_mode=True``.  This is an explicit compatibility/throughput
      probe and is kept separate from the Transformer Engine implementation.
    ...
    elif optimizer_type.lower() in {"apexfusedadam", "apexfusedadamw"}:
        from cosmos_framework.utils.generator.apex_compat import build_apex_fused_adam

        # Apex calls the decoupled-weight-decay mode FusedAdam rather than
        # exposing a separate FusedAdamW class.  The training config already
        # supplies AdamW-compatible betas/eps/weight_decay; only the PyTorch
        # ``fused`` kwarg is not accepted by Apex.
        optimizer_kwargs.pop("fused", None)
        optimizer_kwargs["adam_w_mode"] = True
        optimizer = build_apex_fused_adam(params, **optimizer_kwargs)
    else:
        raise NotImplementedError(f"Optimizer {optimizer_type} not found.")
    log.info(f"Created optimizer implementation: {type(optimizer).__module__}.{type(optimizer).__name__}")
    return optimizer
